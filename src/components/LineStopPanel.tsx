...
export default function LineStopPanel() {
  ...
  // living only inside that modal. CATEGORY (AV/PE/RQ) is a line-stop-only
  // column; INFORMATION is a separate note-only column — the two are never
  // mixed, even though both render blank for the row kind they don't apply to.
  const rows: Row[] = [
  ...
  return (
    <div className="text-white text-xs">
      ...
      <table className="w-full text-[11px]">
        ...
        <tbody>
          {rows.length === 0 && (
            <tr><td colSpan={7} className="px-2 py-2 text-gray-500">Belum ada line stop / informasi.</td></tr>
          )}
          {rows.map((row) => {
            if (row.kind === 'note') {
              const { note } = row;
