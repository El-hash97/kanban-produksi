...
describe('deriveTappingGroups', () => {
  ...
  it('assigns furnaces on the fixed F1,F1,F3,F4,F3,F2,F2 cycle, F3 slots pulling KAI/CRANK', () => {
    // Kanban laid out in the same order the cycle would consume it, so the
    // sequence and the cycle coincide here.
    const lots = [
      ...makeLots('2TR', 6, 400), // F1, F1
      ...makeLots('CRANK', 3, 424), // F3
      ...makeLots('2TR', 3, 436), // F4
      ...makeLots('KAI', 3, 448), // F3
      ...makeLots('2TR', 6, 460), // F2, F2
    ];
  ...
  it('orders taps by kanban position, not by cycle slot — a KAI tap keeps F3 but takes its real place', () => {
    // Reported bug: the cycle pulls two TR taps (F1, F1) before its F3 slot,
    // so a KAI group that the kanban places right after the first TR group
    // was being numbered *after* the next two TR groups instead.
    const lots = [
      ...makeLots('2TR', 3, 400), // kanban #1
      ...makeLots('KAI', 3, 412), // kanban #2  <- must be sequence 2, still furnace 3
      ...makeLots('2TR', 3, 424), // kanban #3
      ...makeLots('2TR', 3, 436), // kanban #4
    ];
    const groups = deriveTappingGroups(lots);
    expect(groups.map((g) => g.lots[0].productCode)).toEqual(['2TR', 'KAI', '2TR', '2TR']);
    expect(groups.map((g) => g.sequenceNo)).toEqual([1, 2, 3, 4]);
    expect(groups.map((g) => g.furnaceId)).toEqual([1, 3, 1, 4]);
  });

  it('falls back to TR (still furnace 3, 2x in the cycle) once KAI/CRANK runs out mid-pass', () => {
    const lots = [
      ...makeLots('CRANK', 3, 400), // only 1 furnace-3 group available, first on the kanban
      ...makeLots('2TR', 18, 500), // enough TR to fill every other slot, including the 2nd F3 slot
    ];
    const groups = deriveTappingGroups(lots);
    expect(groups).toHaveLength(7);
    // CRANK leads the kanban so it's sequence 1 (on F3); the TR taps follow
    // on their own cycle, with the 2nd F3 slot (index 4) falling back to TR.
    expect(groups.map((g) => g.furnaceId)).toEqual([3, 1, 1, 4, 3, 2, 2]);
    expect(groups[0].shape).toBe('triangle');
  ...
  it('once KAI/CRANK is fully done before a new pass starts, furnace 3 taps 2x back-to-back (no F4 gap)', () => {
    ...
    expect(groups.map((g) => g.furnaceId)).toEqual([
      3, 1, 1, 4, 3, 2, 2, // CRANK leads the kanban; 1st TR pass: split cycle (F3 fallback at index 4)
  ...
  it('numbers sequenceNo in kanban order starting at 1', () => {
    const lots = [
      ...makeLots('2TR', 15, 400),
